...
router.get('/user/:userId', async function (req, res) {
    try {
        ...
        return res.status(500).json({ error: `An error occurred while fetching notificatons: ${error.message}` });
    }
});
...
router.delete('/:notificationId', 
    ...
    try {
        ...
        if (req.user.id != user._id.toString())
            return res.status(404).json({error: "No permissions to remove notification"});
        ...
        res.status(500).json({error: `Error while deleting notification: ${error.message}`});
    }
})
